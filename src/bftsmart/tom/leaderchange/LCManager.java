 ...
 * protocol
 * It also implements some predicates and methods necessary for the protocol in
 * accordance
...
public class LCManager {
    ...
    public boolean sound(HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) { // organize all existing timestamps and values separately
            ...
            logger.debug("Context for replica " + c.getPid() + ": CID[" + c.getCid() + "] WRITESET[" + c.getWriteSet()
                    + "] (VALTS,VAL)[" + c.getQuorumWrites() + "]");
        ...
        for (int r : timestamps) {
            for (byte[] v : values) {
                ...
                logger.debug(
                        "testing predicate BIND for timestamp/value pair (" + r + " , " + Arrays.toString(v) + ")");
                if (binds(r, v, collects)) {

                    logger.debug(
                            "Predicate BIND is true for timestamp/value pair (" + r + " , " + Arrays.toString(v) + ")");
     ...
     *         Also, see pages 252 and 253 from "Introduction to Reliable and Secure
    ...
    public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        if (appears)
            logger.debug("timestamp/value pair (" + timestamp + " , " + Arrays.toString(value)
        ...
        if (quorum)
            logger.debug("timestamp/value pair (" + timestamp + " , " + Arrays.toString(value) +
                    ") has the highest timestamp among a "
    ...
    public boolean hasValidProof(CertifiedDecision cDec) {
        ...
        return countValid >= (certificateLastView != -1 && pubKey != null ? certificateLastView
