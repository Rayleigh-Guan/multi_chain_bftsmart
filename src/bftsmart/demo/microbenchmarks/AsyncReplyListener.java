...
public class AsyncReplyListener implements ReplyListener {
    ...
    public void printStaticsInfo() {
        ...
        for (HashMap.Entry<Integer, Long> entry : replyRecvTime.entrySet()) {
            ...
            if (reqSendTime.containsKey(reqId) == false)
                ...
                System.out.println(this.id + " " + reqId + " " + reqSendTime.get(reqId) + " "
                        + reqCompletedSendime.get(reqId) + " " + recvTime);
            }
    ...
    public void replyReceived(RequestContext context, TOMMessage reply) {
        ...
        // double q = Math.ceil((double)
        // (serviceProxy.getViewManager().getCurrentViewN() +
        // serviceProxy.getViewManager().getCurrentViewF() + 1) / 2.0);

        // if (replies >= q) {
        // if (verbose) System.out.println("[RequestContext] clean request context id: "
        // + context.getReqId());
        // serviceProxy.cleanAsynchRequest(context.getOperationId());
        // }
    }
