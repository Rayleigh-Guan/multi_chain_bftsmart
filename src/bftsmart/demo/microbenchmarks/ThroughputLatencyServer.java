...
public final class ThroughputLatencyServer extends DefaultRecoverable {
    ...
    public byte[][] appExecuteBatch(byte[][] commands, MessageContext[] msgCtxs, boolean fromConsensus) {
        ...
        System.out.printf(
                "Node %d execute a Batch at time %d, execute interval %d ms, batch txes num: %d, TPS: %d, total txes num: %d\n",
    ...
    public byte[] execute(byte[] command, MessageContext msgCtx) {
        ...
        if (iterations % interval == 0) {
            ...
            System.out.println("Total latency = " + totalLatency.getAverage(false) + " (+/- "
                    + (long) totalLatency.getDP(false) + ") us ");
            totalLatency.reset();
            System.out.println("Consensus latency = " + consensusLatency.getAverage(false) + " (+/- "
                    + (long) consensusLatency.getDP(false) + ") ms ");
            consensusLatency.reset();
            System.out.println("Pre-consensus latency = " + preConsLatency.getAverage(false) + " (+/- "
                    + (long) preConsLatency.getDP(false) + ") ms ");
            preConsLatency.reset();
            System.out.println("Pos-consensus latency = " + posConsLatency.getAverage(false) + " (+/- "
                    + (long) posConsLatency.getDP(false) + ") ms ");
            posConsLatency.reset();
            System.out.println("Propose latency = " + proposeLatency.getAverage(false) + " (+/- "
                    + (long) proposeLatency.getDP(false) + ") ms ");
            proposeLatency.reset();
            System.out.println("Write latency = " + writeLatency.getAverage(false) + " (+/- "
                    + (long) writeLatency.getDP(false) + ") ms ");
            writeLatency.reset();
            System.out.println("Accept latency = " + acceptLatency.getAverage(false) + " (+/- "
                    + (long) acceptLatency.getDP(false) + ") ms ");
            acceptLatency.reset();
