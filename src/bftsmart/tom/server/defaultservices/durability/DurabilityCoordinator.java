 ...
 * instead of
 * all replicas send the state and log, or state and hash of the log, the
 * replicas
 * are divided into three groups. One group sends the state, another group sends
 * the
 * lower half of the log and the third group sends the upper portion of the log.
 * The replica that receives the state, logs and hashes validates the state and
 * install
 * it.
 * The details of the protocol are described in the paper "On the Efficiency of
 * Durable
...
public abstract class DurabilityCoordinator implements Recoverable, BatchExecutable {
	 ...
	 * When a replica take a checkpoint, it is necessary to save in an auxiliary
	 * table
	 ...
	 * several
	 * consensus instances can be executed in the same batch of commands, it is
	 * necessary
	 ...
	 *         there is
	 *         no replica taking a checkpoint during the period comprised by this
	 *         command batch, it
	 *         is returned -1
	 */
	 ...
	 * command
	 * being executed. As several times during the execution of commands and logging
	 * the
	 * only information important in MessageContext is the consensus id, it saves
	 * time to
