 ...
 * protocol.
 * Typically, classes should both implement this interface and one of the
 * executables.
 *
 */
public interface Recoverable {
     ...
     * 
     * @param cid       Consensus up to which the application should return an
     *                  Application state
     * @param sendState true if the replica should send a complete
     *                  representation of the state instead of only the hash. False
     *                  otherwise
     ...
     * decided in a particular consensus instance. This method is always invoked
     * before
     * the executor. However, multiple invocations for the same consensus ID may
     * occur, so developers must take this behavior into consideration when
     * developing
     ...
     * @param msgCtx   Message context associated with the client request and the
     *                 consensus instance
     *                 where it was ordered. msgCtx.getConsensusId() will be equal
     *                 to CID.
     * 
     * 
     */
     ...
     * only decided a single reconfiguration operation. or an instance where the
     * client
     * operation was not delivered because its view was outdated). To allow the
     * underlying state
     * transfer protocol to execute correctly, it needs to be notified of this
     * special case
     * In the current protocols included, it suffices to register a NOOP operation
     * in the
     * logs used within the state transfer, but never deliver it to the application
     * 
     * @param CID        the consensus instance where the aforementioned condition
     *                   occurred
     * @param operations Operations decided in CID
     * @param msgCtx     Message context associated with the consensus instance.
     *                   furthermore
